...
FROM Filmes AS F
JOIN FilmesGenero AS FG ON F.Id = FG.FilmeId
JOIN Generos AS G ON FG.GeneroId = G.Id;
...
FROM Filmes AS F
JOIN ElencoFilme AS EF ON F.Id = EF.FilmeId
JOIN Atores AS A ON EF.AtorId = A.Id;
